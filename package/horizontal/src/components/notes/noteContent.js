...
const NoteContent = () => {
	const noteDetails = useSelector((state) => state.notesReducer.notes[state.notesReducer.notesContent - 1]);
	let title, color;
	...
	return (
        ...
        <input className="d-none" type="text"  name="title"  id="title"  ref={node => (title = node)} />
        <input className="d-none" type="text" name="color" id="color" ref={node => (color = node)} />
        	<Button className="btn btn-success ml-auto" size="sm" 
        		onClick={e => {
	            	e.preventDefault();
	                    dispatch(
	                        addNote(
        ...
        { noteDetails && noteDetails.deleted === false ? (
        	...
        	<Input type="select" className="col-md-2" name="select" value={noteDetails.color}  onChange={e => dispatch(updateNote(noteDetails.id, "color", e.target.value))}>
