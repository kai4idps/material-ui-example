...
const NoteList = () => {
    ...
    const notes = useSelector((state) => filterNotes(state.notesReducer.notes, state.notesReducer.noteSearch));
                ...
                {notes && notes.length ?
                    notes.map(note => (
                        <Note
                            ...
                            noteDatef={(new Date(note.datef)).toLocaleDateString({ weekday: 'short', year: 'numeric', month: 'short' })}
                        />
                    ))
                    : "no notes"}
            </div>
